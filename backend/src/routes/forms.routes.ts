 ...
 * Generate the printable OBJEDNÁVKA PDF for a persisted ADMF form.
 * Returns application/pdf — replaces the legacy PNG export.
 */
router.get("/:id/pdf", authenticateToken, async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const safeName = `objednavka-${form.id}.pdf`;
