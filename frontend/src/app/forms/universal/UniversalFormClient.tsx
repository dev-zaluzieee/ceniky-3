...
export default function UniversalFormClient({
  ...
  // Pair validation state (must be 1 Raynet + 1 ERP, no conflicts)
   ...
   * Prefill values are validated as "Raynet + ERP match and no conflicts".
   */
   ...
   * Always shows results to user for selection (must select 1 Raynet + 1 ERP).
   */
  ...
  /**
   * Validate the selected Raynet+ERP pair and apply prefill when OK.
   */
  const handleValidateAndApply = async () => {
    ...
    try {
      ...
      if (!result.data.ok) {
        ...
        // Apply resolved prefill if available, but keep the selection open so user can choose a different pair.
