...
export default async function TextileRoletyFormEditPage({
  ...
  if (form.form_type !== "textile-rolety") {
    // Form type mismatch - redirect to forms list
    redirect("/forms/list?error=invalid_form_type");
  }

  // Transform form_json to TextileRoletyFormData
  // The form_json should already match TextileRoletyFormData structure
  const initialData = form.form_json as TextileRoletyFormData;
  ...
  return (
    <TextileRoletyFormClient initialData={initialData} formId={formId} />
  );
}
