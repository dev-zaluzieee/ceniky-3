...
/**
 * Window/door screens form edit page - Server Component
 * Fetches form data and renders the Client Component in edit mode
 */
export default async function SiteFormEditPage({
  params,
}: {
  ...
  if (form.form_type !== "site") {
    // Form type mismatch - redirect to forms list
    redirect("/forms/list?error=invalid_form_type");
  }

  // Transform form_json to SiteFormData
  // The form_json should already match SiteFormData structure
  const initialData = form.form_json as SiteFormData;
  ...
  return <SiteFormClient initialData={initialData} formId={formId} />;
}
